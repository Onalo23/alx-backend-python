...
class TestGithubOrgClient(unittest.TestCase):
    ...
    def test_public_repos(self, get_json_mock):
        ...
        jerry = {"name": "Jerry", "license": {"key": "a"}}
        bob = {"name": "Bob", "license": {"key": "b"}}
        sunday = {"name": "Sunday"}
        to_mock = 'client.GithubOrgClient._public_repos_url'
        get_json_mock.return_value = [jerry, bob, sunday]
        with patch(to_mock, PropertyMock(return_value="www.yes.com")) as y:
            x = GithubOrgClient("x")
            self.assertEqual(x.public_repos(), ['Jerry', 'Bob', 'Sunday'])
            self.assertEqual(x.public_repos("a"), ['Jerry'])
